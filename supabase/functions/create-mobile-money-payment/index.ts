...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Checkout UIs that use the hosted Payment Page do not send country/operator; country is resolved in create-pawapay-payment.
    if (!hasWalletRouting && toggles.pawapay) {
      return await forwardTo("create-pawapay-payment");
    }
    ...
    if (!hasWalletRouting) {
      if (cc.length !== 2) {
        return new Response(
          JSON.stringify({
            error: "countryCode is required (ISO 3166-1 alpha-2, e.g. GH).",
          }),
    ...
    for (const w of warnings) console.warn("[mobile-money]", w);
    ...
    if (toggles.pawapay && toggles.dusupay && provider === "dusupay") {
      console.warn(
        "[mobile-money] Routed to DusuPay (PawaPay does not support this country/operator or cache unavailable).",
    ...
    const fn = provider === "pawapay" ? "create-pawapay-payment" : "create-dusupay-payment";
